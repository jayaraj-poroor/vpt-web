        ...
        if (resp.status != 200) {
            ...
            doPost("/signin", user, function (resp) {
                if (resp.status == 401) {
                    ...
                    bootbox.alert("Server Error: " + resp.status + "(" + (typeof resp.msg == "string"? resp.msg : "Unknown") + "). Please try again.");
                    console.log(resp);
                }
            }, function (err) {
...
function signup(id, name, email, password) {
    ...
    doPost("/signup", user, function (resp) {
        removeWaitingOverlay();
        if (resp.status != 200) {
            bootbox.alert("Server Error: " + resp.status + "(" + (typeof resp.msg == "string"? resp.msg : "Unknown") + "). Please try again later.", function () {
                window.location = window.actualSite + "/";
            });
            console.log(resp);
        }
